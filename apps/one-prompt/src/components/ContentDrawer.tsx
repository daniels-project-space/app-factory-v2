...
// Mini Mood Graph Component
function MiniMoodGraph({ entries, theme }: { entries: JournalEntry[]; theme: ReturnType<typeof useAppTheme> }) {
  const weekStart = getWeekStart(new Date());
  const moodData = useMemo(() => generateWeeklyMoodData(entries, weekStart), [entries, weekStart]);
...
export function ContentDrawer({ onClose }: ContentDrawerProps) {
  ...
  // Get this week's data for insights
            ...
            >
              ...
              {isPremium ? (
                <>
                  ...
                  {/* Mood tracking graph (if enabled) */}
                  {moodTrackingEnabled && (
